   ...
   /**/     
         timeSlider.setOnMouseDragged(new EventHandler <MouseEvent>() {
            public void handle(MouseEvent mouseEvent) {       
                 ...
                 Duration slideTo = Duration.millis(ms);
                 mediaPlayer.seek(slideTo);
                 currentText.setText(rebuild(mediaPlayer.getCurrentTime()));
            }
         });
    }
    
       private String rebuild(Duration duration) {
        double millis = duration.toMillis();
        int ss = (int) (millis / 1000) % 60;
        int ms = (int) (millis / (1000 * 60));
